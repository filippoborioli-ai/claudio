...
;(function (root) {
    ...
    el.appendChild(ui.panel('Flusso di lavoro tipico', null,
      h('div', { class: 'steps' }, [
        ['Carica i dati', 'Dalla barra in alto: <b>Importa</b> per file CSV/TXT/Excel, oppure incolla direttamente da Excel (Ctrl+V anche fuori dalle caselle di testo). ' +
          'Con <b>Esempi</b> carichi dataset gia pronti per provare ogni analisi.'],
        ...
        ['Analizza', 'Test di ipotesi, ANOVA, regressione, carte di controllo, capacita, MSA, DoE: ogni vista ha i comandi a sinistra e i risultati a destra.'],
    ...
    el.appendChild(ui.panel('Dove sono i dati', null,
      h('div', { class: 'doc' }, [
        h('p', { html: 'Tutti i calcoli avvengono <b>nel tuo browser</b>: nessun dato viene inviato in rete. ' +
          'Il progetto viene salvato automaticamente nella memoria locale del browser (localStorage) e puo essere ' +
  ...
  function glossario(el) {
    var terms = [
      ['alpha (livello di significativita)', 'Probabilita di dichiarare una differenza che non esiste (falso allarme). Tipicamente 0,05.'],
      ['p-value', 'Probabilita di osservare dati estremi come i nostri se l ipotesi nulla fosse vera. Piccolo = i dati sono poco compatibili con H0. Non e la probabilita che H0 sia vera.'],
      ['Potenza', 'Probabilita di rilevare una differenza che esiste davvero. Cresce con la numerosita e con la grandezza dell effetto, cala con la variabilita.'],
      ['Intervallo di confidenza', 'Campo di valori compatibili con i dati al livello di confidenza scelto. Piu informativo del solo p-value.'],
      ['Intervallo di predizione', 'Campo entro cui cadra una singola nuova osservazione: sempre piu ampio dell intervallo di confidenza della media.'],
      ['Cp e Cpk', 'Capacita potenziale (Cp, ignora il centraggio) ed effettiva (Cpk) calcolate con la sigma di breve termine.'],
      ['Pp e Ppk', 'Gli stessi indici calcolati con la variabilita complessiva di lungo periodo: e quella che vede il cliente.'],
      ['Sigma within e overall', 'Within = variabilita entro sottogruppo (breve termine). Overall = variabilita totale, comprende gli spostamenti nel tempo.'],
      ['DPMO', 'Difetti per milione di opportunita.'],
      ['Livello sigma', 'Numero di deviazioni standard fra la media e il limite di specifica piu vicino, per convenzione con uno spostamento di 1,5 sigma.'],
      ['Causa comune / speciale', 'Comune = variabilita intrinseca del processo (sistema). Speciale = evento identificabile e rimovibile.'],
      ...
      ['Gage R&R', 'Quota di variabilita totale dovuta al sistema di misura (ripetibilita + riproducibilita).'],
      ...
      ['Alias', 'Due effetti che non possono essere distinti perche stimati dalla stessa combinazione di prove.'],
      ['Risoluzione', 'Indica quali effetti sono confusi fra loro in un disegno frazionario.'],
      ['Desiderabilita', 'Punteggio da 0 a 1 che traduce quanto una risposta si avvicina all obiettivo; la composita e la media geometrica.'],
      ['R-quadro previsto', 'Quota di variabilita che il modello spiegherebbe su nuovi dati, calcolata con la validazione leave-one-out (PRESS).'],
      ...
      ['OEE', 'Disponibilita x prestazione x qualita: efficacia complessiva dell impianto.'],
  ...
  function validazione(el) {
    el.appendChild(ui.panel('Come sono stati verificati i calcoli', {
      ...
      h('table', { class: 'data' }, [
        ...
        h('tbody', null, [
          ...
          ['Capacita', 'Cp = 1 e Cpk = 1 producono esattamente 2699,8 PPM; la conversione 6 sigma <-> 3,4 DPMO e verificata.'],
          ...
          ['Disegni sperimentali', 'Ortogonalita di tutti gli array (X\'X diagonale) verificata a runtime; risoluzione e alias calcolati dal gruppo dei contrasti definenti; conteggi di prove di CCD e Box-Behnken confrontati con i riferimenti.'],
    ...
    el.appendChild(ui.panel('Convenzioni adottate', null, ui.kv([
      ['Percentili', 'Interpolazione (n+1)p, la stessa usata dai principali software di qualita. La mediana usa il metodo classico.'],
      ...
      ['Effetti nel DoE', 'Effetto = 2 x coefficiente in unita codificate.'],
      ['Capacita non normale', 'Metodo ISO sui percentili 0,135% e 99,865% della distribuzione adattata.'],
    ...
    el.appendChild(ui.panel('Riferimenti', null,
      h('div', { class: 'doc' }, h('ul', null, [
        ...
        'D. C. Montgomery, Introduction to Statistical Quality Control (carte di controllo, capacita, campionamento).',
        'AIAG, Measurement Systems Analysis (Gage R&R, bias, linearita, concordanza per attributi).',
        'ASTM E2587 / manuali ASTM per le costanti delle carte di controllo.',
        'G. Derringer, R. Suich, Simultaneous Optimization of Several Response Variables (desiderabilita).',
