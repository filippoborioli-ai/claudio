...
;(function (root) {
  ...
  C3.app.registerView({
    ...
    icon: '⚑',
    group: 'Metodologie',
    desc: 'Quale disegno sperimentale usare e perche: strategia per fasi, selettore guidato, concetti chiave, procedura di analisi ed errori da evitare.',
    render: function (el) {
      var tabs = ui.tabs([
  ...
  function strategia(el) {
    el.appendChild(ui.panel('La sperimentazione e sequenziale', {
      sub: 'mai spendere tutto il budget in un unico esperimento'
    }, h('div', { class: 'doc' }, [
      h('p', { html: 'Regola pratica consolidata: <b>non piu del 25% del budget sperimentale nel primo esperimento</b>. ' +
        'Un esperimento serve a imparare e a decidere il successivo: la conoscenza arriva per cicli, non in un colpo solo.' }),
      h('p', { html: 'La sequenza tipica e: <b>screening</b> (quali fattori contano) -> <b>caratterizzazione</b> ' +
      ...
      h('div', { class: 'timeline' }, [
        {
          ...
          d: 'Si accetta di confondere le interazioni pur di provare molti fattori con poche prove. ' +
            'L obiettivo non e la precisione ma la selezione: tipicamente 2-5 fattori sopravvivono.',
    ...
    el.appendChild(ui.panel('Metodo della massima pendenza (steepest ascent)', null,
      h('div', { class: 'doc' }, [
        h('p', { html: 'Se l ottimo non e dentro la regione esplorata, il modello lineare indica la <b>direzione</b> in cui muoversi: ' +
          'e la direzione dei coefficienti (gradiente). Si eseguono prove singole lungo quella direzione finche la risposta smette di migliorare; ' +
          'li si centra un nuovo esperimento fattoriale.' }),
        h('p', { html: 'Segnali che si e arrivati vicino all ottimo: gli effetti principali diventano piccoli, la <b>curvatura</b> diventa significativa, ' +
    ...
    function run() {
      ...
      if (v.goal === 'mixture') {
        ...
        recs.push({
          ...
          why: v.nuisance
            ? 'Il blocco cattura la variabilita nota (giorno, lotto, macchina) e la toglie dall errore: il confronto diventa piu sensibile.'
            : 'Con una sola fonte di variazione basta replicare e randomizzare: si analizza con ANOVA e confronti multipli.',
          how: 'Analisi: ANOVA a una via, poi Tukey per le coppie. Verifica varianze uguali (Levene) e normalita dei residui.',
          score: 100
        });
        if (v.k >= 3 && v.nuisance) {
          recs.push({
        ...
        recs.push({
          ...
          why: 'Alternativa moderna: si includono i fattori di rumore nel disegno e si cercano le interazioni controllo x rumore, ' +
            'che sono esattamente cio che rende (o non rende) robusto il processo.',
        ...
        if (v.k <= 5) {
          recs.push({
            ...
            why: 'Stima il modello quadratico completo e permette di trovare massimi, minimi e creste. ' +
              'Se hai gia fatto un fattoriale, basta aggiungere i punti assiali e centrali.',
            how: 'Alpha rotatabile per varianza di previsione uniforme; alpha = 1 (face-centered) se non puoi uscire dai livelli gia usati.',
            score: 100
          });
          recs.push({
            name: 'Box-Behnken',
        ...
        if (v.k >= 6) {
          recs.push({
            ...
            why: 'Tre livelli per fattore: gli effetti principali non sono confusi con le interazioni a due fattori e ' +
              'la curvatura e stimabile. Con pochi fattori attivi si puo anche ottimizzare senza altre prove.',
            how: 'Ottimo primo esperimento quando i fattori sono 6-12 e si sospetta non linearita.',
            score: 100
          });
          recs.push({
      ...
      if (v.nuisance && v.goal !== 'compare') {
        recs.push({
          ...
          why: 'La fonte di disturbo (giorno, lotto, turno) viene inserita come blocco: la sua variabilita esce dall errore ' +
            'e gli effetti diventano piu facili da rilevare.',
      ...
      if (v.destructive) {
        recs.push({
          ...
          runs: '-',
          why: 'Con prove distruttive non esistono misure ripetute sullo stesso pezzo: la ripetibilita si stima solo con pezzi ' +
      ...
      if (v.levels === 'cat' && ['screening', 'effects'].indexOf(v.goal) >= 0) {
        recs.push({
          ...
          why: 'Con fattori categorici a piu di due livelli il disegno a 2 livelli non basta: serve un fattoriale generale ' +
      ...
      out.appendChild(ui.panel('Quante prove servono davvero', null, [
        ui.verdict('La potenza dipende dal rapporto <b>effetto / deviazione standard</b>. Con ' + Math.pow(2, v.k) +
          ...
          num.fmt(pw.detectable, 2) + ' volte sigma</b>. Per rilevare effetti piu piccoli servono repliche: ' +
  ...
  function tecniche(el) {
    var rows = [
      ...
      },
      {
        ...
        use: '6-12 fattori continui con sospetta non linearita.'
      },
      {
        name: 'Central Composite Design', fase: 'Ottimizzazione', runs: '2^k + 2k + centrali',
        pro: 'Modello quadratico completo, si costruisce aggiungendo punti a un fattoriale gia fatto.',
      ...
      },
      {
        ...
        pro: 'Semplice, aumenta la sensibilita rimuovendo la variabilita del blocco.',
        con: 'Serve che ogni blocco contenga tutti i trattamenti.',
        use: 'Confronto di materiali o metodi su piu giorni o lotti.'
      },
      {
        name: 'Blocchi incompleti bilanciati', fase: 'Confronto', runs: 'variabile',
        pro: 'Funziona quando il blocco non puo contenere tutti i trattamenti.',
        con: 'Analisi piu complessa: le medie semplici sono distorte.',
      ...
      },
      {
        ...
        use: 'Temperatura del forno, velocita della linea, fattori di setup lungo.'
      },
      {
      ...
      },
      {
        ...
        con: 'Non ha struttura elegante: le proprieta vanno verificate numericamente.',
      ...
      },
      {
        ...
        use: 'Ottimizzazione continua di processi gia stabili e ad alto volume.'
      }
    ];
  ...
  function concetti(el) {
    var items = [
      {
        ...
        text: 'Eseguire le prove in ordine casuale distribuisce l effetto di fattori non controllati (deriva termica, ' +
          ...
          'E l unica difesa contro le cause sconosciute e rende validi i test statistici.'
      },
      {
        name: 'Replicazione (non ripetizione)',
        text: 'Replicare significa rifare la prova completa, setup compreso. Ripetere significa misurare piu volte lo stesso pezzo. ' +
          'Solo la replicazione stima l errore sperimentale vero; usare ripetizioni come repliche fa sembrare il processo ' +
          'molto piu preciso di quanto sia e porta a dichiarare significativi effetti inesistenti.'
      },
      {
        name: 'Blocchi',
        text: 'Un blocco e una fonte di variabilita nota ma non interessante (giorno, lotto, macchina). ' +
          'Metterla nel modello la toglie dall errore e rende piu facile vedere gli effetti dei fattori. ' +
          'Principio: blocca cio che puoi controllare ma non ti interessa, randomizza il resto.'
      },
      {
        ...
        text: 'Nei disegni frazionari alcuni effetti sono stimati dalla stessa combinazione di dati: si dice che sono confusi (alias). ' +
          'La relazione definente elenca tutte le coppie confuse. Non e un difetto, e il prezzo pagato per usare meno prove: ' +
      ...
      },
      {
        ...
        text: 'Prove alla media di tutti i fattori. Servono a tre cose: stimare l errore puro senza replicare tutto il disegno, ' +
          ...
          'e controllare la stabilita del processo durante l esperimento.'
      },
      {
        name: 'Effetto e coefficiente',
        text: 'In unita codificate (-1, +1) l effetto e la differenza fra la media al livello alto e quella al livello basso. ' +
          'Il coefficiente del modello vale esattamente la meta dell effetto. Nei confronti fra software questa e la prima ' +
          'fonte di apparenti discordanze.'
      },
      {
        name: 'Metodo di Lenth',
        text: 'Quando il disegno non e replicato non esiste una stima dell errore. Il metodo di Lenth la ricava dagli effetti stessi, ' +
      ...
      {
        name: 'Unita codificate',
        text: 'Codificare i fattori (-1, +1) rende i coefficienti confrontabili fra loro e riduce la collinearita fra termini ' +
          'lineari e quadratici. Le conclusioni si traducono poi in unita reali per l operatore.'
      },
      {
        name: 'Lack-of-fit',
        text: 'Confronta la variabilita non spiegata dal modello con l errore puro stimato dalle repliche. ' +
          'Se e significativo, il modello ha la forma sbagliata (manca curvatura o un interazione), non e un problema di rumore.'
      },
      {
        name: 'Prova di conferma',
        text: 'Un esperimento non finisce con il modello: finisce con prove alle condizioni scelte. ' +
          'Se il risultato cade nell intervallo di predizione il modello e utilizzabile; altrimenti qualcosa e cambiato ' +
    ...
    el.appendChild(ui.panel('I sette passi della pianificazione', { sub: 'da seguire prima di toccare la macchina' },
      h('div', { class: 'steps' }, [
        ['Riconoscere e formulare il problema', 'Che decisione prenderemo con i risultati? Se non e chiara, l esperimento non serve.'],
        ['Scegliere la risposta', 'Misurabile, continua se possibile, legata al problema del cliente. Verifica prima il sistema di misura (Gage R&R): se il 30% della variabilita e dello strumento, l esperimento misura il rumore.'],
        ['Scegliere fattori, livelli e campo', 'Livelli abbastanza distanti da produrre un effetto visibile, ma dentro il campo praticabile. Livelli troppo vicini sono la causa piu comune di esperimenti senza risultati.'],
  ...
  function analisi(el) {
    el.appendChild(ui.panel('Procedura di analisi di un fattoriale', null,
      h('div', { class: 'steps' }, [
        ...
        ['Stima gli effetti', 'Pareto degli effetti standardizzati e normal plot: gli effetti attivi si staccano dalla retta dei punti inattivi.'],
        ...
        ['Verifica curvatura e lack-of-fit', 'Punti centrali e repliche dicono se la forma del modello e adeguata.'],
        ...
        ['Ottimizza', 'Se serve, passa alla superficie di risposta e alla desiderabilita per piu risposte.'],
  ...
  function errori(el) {
    el.appendChild(ui.panel('Errori che rovinano un esperimento', null,
      h('div', { class: 'cards' }, [
        ['Cambiare un fattore alla volta (OFAT)', 'Richiede piu prove, non trova le interazioni e da un ottimo falso. E il motivo principale per cui esiste il DoE.'],
        ...
        ['Ignorare il sistema di misura', 'Se il Gage R&R supera il 30% la variabilita misurata e per lo piu dello strumento: prima si sistema la misura.'],
        ['Analizzare uno split-plot come randomizzato', 'I fattori difficili da variare risultano significativi molto piu spesso di quanto dovrebbero.'],
    ...
    el.appendChild(ui.panel('Lista di controllo prima di avviare le prove', null,
      h('div', { class: 'doc' }, h('ul', null, [
        ...
        'Le prove di conferma sono gia pianificate?'
